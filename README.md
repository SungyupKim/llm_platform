...
1. **AI Agent** 메뉴 클릭
2. 자연어로 질문 입력 (예: "김성엽은 누구인가요?", "회사 소개해줘")
3. AI가 필요한 도구를 자동으로 선택하여 작업 수행
4. 실시간으로 결과 확인
5. **PDF 업로드**: 채팅 인터페이스에서 직접 PDF 파일 업로드 가능

**지원 도구:**
- 📊 **데이터베이스**: PostgreSQL, MySQL 쿼리 실행
- 📁 **파일시스템**: 파일 읽기/쓰기, 디렉토리 탐색
- 🧮 **계산기**: 수학 연산 수행
- 📚 **RAG 시스템**: PDF 문서 업로드, 검색, 질의응답
...
```
사용자: "김성엽은 누구인가요?"
AI: [자동으로 rag_chat 도구 사용하여 업로드된 문서에서 정보 검색]
    "김성엽은 [문서에서 찾은 정보]..."

사용자: "회사 프로젝트에 대해 알려줘"
AI: [자동으로 rag_chat 도구 사용하여 관련 문서 검색]
    "프로젝트는 [문서 내용 기반 답변]..."
```
...
```
사용자: "직원 테이블 구조를 보여줘"
AI: [자동으로 describe_table 도구 사용]
    "직원 테이블 구조: [테이블 스키마 정보]..."
```
...
### RAG MCP 통합 (v2.0)
- **AI Agent와 RAG 완전 통합**: AI Agent에서 직접 문서 기반 질의응답 가능
- **자연스러운 질문 처리**: "김성엽은 누구인가요?", "회사 소개해줘" 등 자연스러운 질문으로 RAG 자동 활용
- **파일 업로드 UI**: AI Agent 채팅 인터페이스에서 직접 PDF 업로드 가능
- **MCP 서버 최적화**: RAG 기능을 MCP 서버로 구현하여 확장성 향상

### 개선된 사용자 경험
- **스마트 도구 선택**: LLM이 질문 내용을 분석하여 적절한 도구 자동 선택
- **실시간 스트리밍**: 모든 응답이 실시간으로 스트리밍되어 표시
- **에러 처리 개선**: 더 안정적인 오류 처리 및 사용자 피드백
...
- **RAG 통합**: 자연스러운 질문으로 문서 기반 답변 자동 제공
